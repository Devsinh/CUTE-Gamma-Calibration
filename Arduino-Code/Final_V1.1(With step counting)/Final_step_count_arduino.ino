...
const float gearRadius = 3.0;        // Gear radius in cm
...
void loop() {
  ...
  if (motorRunning) {
    ...
    // Update the current position based on distance per step
    if (digitalRead(dirPin) == HIGH) {
      currentPosition -= distancePerStep; // Moving up decreases position
    } else {
      currentPosition += distancePerStep; // Moving down increases position
    }

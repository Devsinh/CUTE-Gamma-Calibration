...
parser.on('data', (data) => {
  ...
  if (message.startsWith('Motion Sensor Value:')) {
    ...
    // Broadcast the motion sensor data to all connected WebSocket clients
    if (wss.clients.size > 0) {
      wss.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
          client.send(JSON.stringify({ type: 'motionData', data: motionSensorData }));
        }
      });
    }
  } else if (message === 'H') {
    ...
    // Broadcast the current position to all connected WebSocket clients
    if (wss.clients.size > 0) {
      wss.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
          client.send(JSON.stringify({ type: 'currentPosition', data: currentPosition }));
        }
      });
    }
...
wss.on('connection', (ws) => {
  ...
  // Send current status and position immediately after connection
  ws.send(JSON.stringify({ type: 'status', data: currentStatus }));
  if (currentPosition !== null) {
    ws.send(JSON.stringify({ type: 'currentPosition', data: currentPosition }));
  }

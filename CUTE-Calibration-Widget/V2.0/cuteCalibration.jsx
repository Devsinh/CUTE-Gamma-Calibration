...
function CalibrationSlider() {
    ...
    useEffect(() => {
        const fetchStatus = async () => {
            try {
                const response = await fetch('http://192.168.44.155:8900/status');
    ...
    useEffect(() => {
        const fetchSmallGraphData = async () => {
            try {
                const response = await fetch('http://192.168.44.155:8900/motion-data');
    ...
    const handleResetCommand = async (input) => {
        try {
            const response = await fetch('http://192.168.44.155:8900/reset', {
                method: 'POST',
                headers: {
    ...
    const fetchCurrentPosition = async () => {
        try {
            const response = await fetch('http://192.168.44.155:8900/motor/current-position');
    ...
    const updateMotorPosition = async (position) => {
        try {
            ...
            const url = `http://192.168.44.155:8900/motor/action/${position}`;
    ...
    const handleSetDelay = async () => {
        ...
        if (modifiedDelay < 50 || modifiedDelay > 58054) {
            console.warn('Modified delay must be between 50 and 4000');
            return;
        }

        try {
            await fetch('http://192.168.44.155:8900/delay', {
                method: 'POST',
    ...
    const handleMotorAction = async (action) => {
        try {
            const response = await fetch(`http://192.168.44.155:8900/motor/${action}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            });

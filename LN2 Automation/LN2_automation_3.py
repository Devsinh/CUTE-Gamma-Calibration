...
bignormalweight = 50 # kg
alert_threshold = 20 # weight threshold for big dewar in kg
alert_threshold1 = targetweight - 3.1 # weight threshold for small dewar in kg
...
def monitor_weights():
    ...
    window_duration = 2 * 3600  # 2 hours in seconds
    alert_threshold_minutes = 30  # Alert if filling exceeds 30 minutes in window
    ...
    while True:
        ...
        # Remove fill times older than 2 hours
        ...
        if small_weight < alert_threshold1:
            ...
            while small_weight < 0:
                ...
                # Calculate total fill time in last 2 hours
                total_fill_minutes = len(fill_times)
                
                # Check if we should send an alert
                if total_fill_minutes >= alert_threshold_minutes:
                    alert_message = (
                        f"Warning: System has been filling for {total_fill_minutes} minutes "
                        f"in the last 2 hours. This exceeds the {alert_threshold_minutes} "
                ...
                if small_weight >= target_weight:
                    print("Small dewar weight is at or above 16 kg. Stopping fill cycle.")
                    break

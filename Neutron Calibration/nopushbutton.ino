...
bool overrideOn = false; // To control which motor function is enabled, regular or overridden
bool motorControlEnabled = false; // To control whether the motor can move or not
bool direction = true; // To control the direction, true counterclockwise, false for clockwise
...
// Continuous loop through these functions
void loop()
{
  overrideOn ? motorFunctionOverriden() : motorFunction(); // Use regular motor function when override is off, Use overridden motor function when override is on
...
void motorFunction()
{
  ...
  if(digitalRead(inductionSensorPin1) <= 10 && digitalRead(inductionSensorPin2) <= 10) // Check if the voltage in the induction sensor is less than 10
  {
    ...
    {  
      ...
      else // Check if motor control is not allowed
      {
        digitalWrite(onoffPin, HIGH); // Set motor on
    ...
    else  // Check if the voltage in the induction sensor is greater than 10
    {
...
// Overridden function to control the motor
...
void ChangeDirection()
{
  ...
  digitalWrite(dirPin, direction ? HIGH : LOW); // If direction is true, set dirPin to high, if it's false, set dirPin to low
...
void setPosition() 
{
  sourcePosition = direction ? sourcePosition + distancePerStep : sourcePosition - distancePerStep; // Increase the position if direction is true, decrease if false
}
...
void updatePosition()
{
  ...
  {
    Serial.print("sourcePosition:"); // If the condition is true, update the position on the Webapp
    Serial.println(sourcePosition);
